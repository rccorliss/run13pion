...
indexDisplayName.push_back(Form(%d,newIndex));
  ...
  */
  ...
  for (int i=0;i<nIndices && i<1;i++){
	...
	continue;
     ...
     cTree->Draw("feecore","north");
      ...
      tex.DrawLatex(htemp->GetBinLowEdge(1),htemp->GetMaximum()*0.9,Form("MaxFee between %d and %d",maxfeelow,maxfeehigh));
      pane++;
     c->cd(pane);
      cTree->Draw("feecore",Form("feecore>=%d && feecore<%d && north",maxfeelow,maxfeehigh));
      htemp=(TH1F*)(c->cd(pane)->GetPrimitive("htemp"));
      tex.SetTextSize(0.08);
      ...
      hHotFeeFill->Fill(thisFill,maxfee);
       pane++;
     c->cd(pane);
      cTree->Draw("feecore","ecore>1000 && north");
     htemp=(TH1F*)(c->cd(pane)->GetPrimitive("htemp"));
     maxfeelow=htemp->GetBinLowEdge(htemp->GetMaximumBin());
     maxfeehigh=htemp->GetBinLowEdge(htemp->GetMaximumBin()+1);
     tex.DrawLatex(0.1,htemp->GetMaximum()*0.9,Form("MaxFee between %d and %d",maxfeelow, maxfeehigh));
      pane++;
     c->cd(pane);
     cTree->Draw("feecore",Form("ecore>1000 && feecore>=%d && feecore<%d && north",maxfeelow,maxfeehigh));
     htemp=(TH1F*)(c->cd(pane)->GetPrimitive("htemp"));
      maxfee=htemp->GetBinLowEdge(htemp->GetMaximumBin());
      tex.DrawLatex(htemp->GetBinLowEdge(1),htemp->GetMaximum()*0.9,Form("MaxFee= %d",maxfee));
  ...
  hHotFee->Draw();
  tex.SetTextSize(0.08);
  maxfee=hHotFee->GetBinLowEdge(hHotFee->GetMaximumBin());
  tex.DrawLatex(hHotFee->GetBinLowEdge(1),hHotFee->GetMaximum()*0.9,Form("Hottest ID= %d",maxfee));
